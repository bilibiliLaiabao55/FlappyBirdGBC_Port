...
void update_map(void){
    if(state == 1){
        if((scroll_x-48) % 64 == 0){
            set_bkg_tiles(scroll_x/8-2, 0, pipes_WIDTH/8, pipes_HEIGHT/8, pipes_map+(rand() % 5)*2);
        }
        touched=get_vram_byte(get_bkg_xy_addr((BIRD_X+scroll_x)/8, bird_y/8-2));
        //set_bkg_tiles(0, 0, 1, 1, &touched);
        if((touched >= 0x4D)&&(touched !=0x53))state = 2;
    }
...
void main(void){
    ...
    for(temp0 = 0;temp0<5;++temp0)set_sprite_data(temp0*4, 4, bird_tiles[1][temp0]);
    ...
    while(1){
        ...
        if(gravity == 1){
            ...
            if((drop_speed >= 3)&&(drop_speed <= -3)){
                if(drop_speed < 0)drop_speed = -3;
                else if(drop_speed > 0)drop_speed = 3;
            }
            bird_y += drop_speed;
            if((drop_speed <= -2)&&(drop_speed_sub <= -1))move_metasprite_ex(b00_metasprite0, 8, 0, 0, BIRD_X, bird_y);
            else if(drop_speed <= -2)move_metasprite_ex(b00_metasprite0, 4, 0, 0, BIRD_X, bird_y);
            else if((drop_speed >= 2)&&(drop_speed_sub >= 1))move_metasprite_ex(b00_metasprite0, 16, 0, 0, BIRD_X, bird_y);
            else if(drop_speed >= 2)move_metasprite_ex(b00_metasprite0, 12, 0, 0, BIRD_X, bird_y);
            else if((drop_speed < 1)&&(drop_speed > -1))move_metasprite_ex(b00_metasprite0, 0, 0, 0, BIRD_X, bird_y);
        }
        if((cgb_enable == 1)&&(bird_flap_wing == 1)){
            if(bird_frame_delay >= 10){
                ++bird_frame;
                bird_frame_delay = 0;
                if(bird_frame >= 4)bird_frame = 0;
            }
            else bird_frame_delay++;
            for(temp0 = 0;temp0<5;++temp0)set_sprite_data(temp0*4, 4, bird_tiles[bird_frame][temp0]);
        }else {
            bird_frame = 1;
            bird_frame_delay = 0;
        }
